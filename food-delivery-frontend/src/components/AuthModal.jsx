    ...
    try {
      if (mode === "signin") {
        ...
        switch (role) {
          ...
          case "customer":
          default:
            payload = { name, mobileNumber, email, password, roleType: 'CUSTOMER' };
            registrationResponse = await apiPost(`${API_BASE_URL}/api/auth/register/customer`, payload);

            console.log("Customer registration response:", registrationResponse);

            // Check if the registration was successful and gave us what we need
            // This is the most likely point of failure. Adjust this `if` condition
            // based on what you see in the console log above.
           if (registrationResponse && registrationResponse.user?.id && registrationResponse.token) {
    const { label, street, city, state, postalCode, country, isDefault } = formData;

    if (label || street || city || state || postalCode || country) {
        const addressPayload = {
            // It correctly gets the user ID from the response
             ...
             {mode === "signup" && (
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="tel" // `tel` is the appropriate type for phone numbers
                    name="mobileNumber"
